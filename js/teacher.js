...
function updatePaper(start, end) {
  for (var i = start; i < end; i++) {
    var new_tbody = $('<div id="tbody0' + htdata.historyList[i].pid + '" class="row" style="margin-bottom: 20px;border-bottom:rgb(248,248,248) 2px solid;font-weight: bold;width:100%;"><input id="check' + htdata.historyList[i].pid + '" type="checkbox" name="checkbox" class="col-md-1"><div class="row col-md-11"><div class="col-md-2">' + htdata.historyList[i].name + '</div><div class="col-md-2">' + htdata.historyList[i].sid + '</div><div class="col-md-6" style="overflow:hidden;"><a href="download.action?paper_name=' + encodeURIComponent(encodeURIComponent(htdata.historyList[i].paper_id)) + '">' + htdata.historyList[i].paper_id + '</a></div><div class="col-md-2"><a href="download.action?paper_name=report' + encodeURIComponent(encodeURIComponent(htdata.historyList[i].paper_id)) + '">点击下载</a></div></div></div>').appendTo($('#eventLog'));
